...
describe Dog do
  ...
  subject(:bulleit) { described_class.new('bulleit', 'pit-bull', 'white/gray') }
  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'when it shares a method name with another Class' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(bulleit).to_not be_barking
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(bulleit).to be_sleeping
  end
end
