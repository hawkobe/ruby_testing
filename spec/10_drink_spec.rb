...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:latte_drink) { described_class.new('latte') }

      # remove the 'x' before running this test
      it 'is latte' do
        expect(latte_drink.type).to eq('latte')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(latte_drink.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:default_diner_drink) { described_class.new('coca-cola') }

      # remove the 'x' before running this test
      it 'is full' do
        expect(default_diner_drink).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:small_coke) { described_class.new('coca-cola', 8) }

      # remove the 'x' before running this test
      it 'is not full' do
        expect(small_coke).to_not be_full
      end
    end
  end
end
